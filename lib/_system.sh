...
system_nginx_install() {
  print_banner
  printf "${WHITE} 💻 Instalando nginx e o módulo GeoIP2...${GRAY_LIGHT}"
  printf "\n\n"
...
system_geoip_setup() {
  print_banner
  printf "${WHITE} 💻 Baixando e configurando o banco de dados GeoIP...${GRAY_LIGHT}"
  printf "\n\n"
...
wget -O /usr/share/GeoIP/GeoLite2-Country.mmdb.gz "https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-Country&license_key=${maxmind_license_key}&suffix=tar.gz"
gunzip /usr/share/GeoIP/GeoLite2-Country.mmdb.gz

cat > /etc/nginx/conf.d/geoip.conf << 'END'
geoiptwo /usr/share/GeoIP/GeoLite2-Country.mmdb {
    auto_reload 60m;
